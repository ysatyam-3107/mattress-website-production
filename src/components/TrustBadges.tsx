      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 py-10 border-t border-gray-100">
        {badges.map((badge, idx) => (
          <div key={idx} className="flex flex-col items-center text-center p-4 rounded-2xl bg-white border border-gray-50 hover:border-blue-100 transition-colors shadow-sm">
            <div className="w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center mb-3 text-[#2563EB]">
              <badge.icon className="w-6 h-6" />
            </div>
            <p className="font-bold text-[#001166] text-sm font-montserrat">{badge.text}</p>
        ...
        {badges.slice(0, 4).map((badge, idx) => (
          <div key={idx} className="flex gap-3 items-start p-2 rounded-xl bg-gray-50/50 border border-gray-100">
            <div className="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center shrink-0 border border-blue-100">
              <badge.icon className="w-4 h-4 text-[#2563EB]" />
            </div>
            <div className="min-w-0">
              <p className="font-bold text-[#000d44] text-[10px] font-montserrat uppercase tracking-tight leading-tight truncate">{badge.text}</p>
    ...
    <div className={`flex ${variant === "vertical" ? "flex-col gap-6" : "flex-wrap gap-8"} justify-center py-8`}>
      {badges.slice(0, 4).map((badge, idx) => (
        <div key={idx} className="flex items-center gap-4 group">
          <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center shrink-0 border border-blue-100 group-hover:bg-[#2563EB] group-hover:border-[#2563EB] transition-all">
            <badge.icon className="w-5 h-5 text-[#2563EB] group-hover:text-white transition-colors" />
          </div>
          <div>
            <p className="font-bold text-[#000d44] text-xs font-montserrat uppercase tracking-wider">{badge.text}</p>
