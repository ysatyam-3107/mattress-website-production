                ...
                <div className="flex gap-2">
                  {selectedProducts.map((p) => (
                    <div key={p.id} className="group relative">
                      <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl bg-white/20 p-1 border border-white/20 overflow-hidden shadow-inner">
                  ...
                  {[...Array(3 - selectedProducts.length)].map((_, i) => (
                    <div key={i} className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center dashed border-dashed">
                      <LayoutGrid className="w-3 h-3 sm:w-4 sm:h-4 text-white/20" />
                    </div>
                  ))}
                </div>
                <div>
                   <p className="text-xs sm:text-sm font-bold font-montserrat tracking-tight leading-none">{selectedProducts.length} Selected</p>
